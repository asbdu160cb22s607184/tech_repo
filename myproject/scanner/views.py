...
def index(request):
    ...
    return render(request, "myproject.scanner/index.html",context)                     
def about(request):
     return render(request,'myproject.scanner/about.html')
def fa(request):
     return render(request,'myproject.scanner/fa.html')
def api(request):
     return render(request,'myproject.scanner/api.html')
def terms(request):
     return render(request,'myproject.scanner/terms.html')
def docs(request):
     return render(request,'myproject.scanner/docs.html')

def grade(request):
    if request.method == 'POST':
        url = request.POST.get('url')
        if not url:
            return render(request, 'myproject.scanner/index.html', {'error': 'No URL provided'})
        if not url.startswith('http://') and not url.startswith('https://'):
            url = 'http://' + url
        
        if not validators.url(url):
            return render(request, 'myproject.scanner/invalid_url.html', {'error': 'Invalid URL format. Please enter a valid site URL.'})
        try:
            headers_req = {
            ...
            try:
                ...
                return render(request, 'myproject.scanner/invalid_url.html', {'error': 'Failed to resolve domain. Please check the URL.'})
            ...
            try:
                ...
                return render(request, 'myproject.scanner/index.html', {'error': f'Database error: {db_error}'})
            # try:
    ...
    # Update or create all GradeSummary records
                ...
                return render(request, 'myproject.scanner/index.html', {'error': f'Summary update error: {summary_error}'})
            ...
            return render(request, 'myproject.scanner/grade.html', context)

        except Exception as e:
            return render(request, 'myproject.scanner/index.html', {'error': str(e)})

    return render(request,"myproject.scanner/index.html")    
